...
import createChannel from './slack-create-channel'
...
export default function slackin ({
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      ...
      splash({ coc, path, css, name, org, logo, channels, active, total, create })
    )
    res.type('html')
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (create) {
      const newChannelName = 'in-' + moment().format('YYYY-MM-DD-HHmmssSS')
      createChannel({ token, org, name: newChannelName }, (err, newChannel) => {
        if (err) {
        ...
        invite({ token, org, email, channel: newChannel.id }, err => {
          if (err) {
            if (err.message === `Sending you to Slack...`) {
              return res
                .status(303)
          ...
          const welcome = 'Welcome to the Computer Lab slack! How can we help you?';

          postMessage({ token, org, channel: newChannel.id, text: welcome }, err => {
            if (err) console.log(err.message);
          });

          res
            .status(200)
            .json({ msg: 'Invited you to a new channel!' })
        })
