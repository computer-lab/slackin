...
var createchannel = form.elements['createchannel']
...
body.addEventListener('submit', function (ev){
  ...
  var channelValue = channel ? channel.value : null;
  var cocValue = coc && coc.checked ? 1 : 0;
  var createchannelValue = createchannel ? createchannel.value : null;
  invite(channelValue, cocValue, email.value, createchannelValue, function (err, msg){
    if (err) {
...
function invite (channel, coc, email, createchannel, fn){
  request
