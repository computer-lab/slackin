...
export default function createChannel({ org, token, channelName }, fn) {
  let data = { channelName, token }

  request
  .post(`https://${org}.slack.com/api/channel.create`)
  .type('form')
  .send(data)
  .end(function (err, res) {
    ...
    if (!ok) {
      ...
      } else if (providedError === 'name_taken') {
        fn(new Error('A channel cannot be created with the given name.'))
      } else {
